...
import { getOutcomeInsightsByUser, getOutcomeInsightsByJourney, getOutcomeInsightsByCountry, getMedicationOutcomesByMedication, getProviderOutcome, getUserOutcomeSummary, } from './lib/outcomes/query.js';
...
import { createConversation, getConversation, getUserConversations, sendMessage, getConversationMessages, addParticipant, removeParticipant, markAsRead, } from './lib/conversation/service.js';
import { createTask, getTask, getTaskWithDocument, getUserTasks, getAllUserTasksByStatus, updateTaskProgress, executeTranslateDocumentTask, executeExtractMedicalDataTask, executeGenerateSummaryTask, executeExtractConditionsTask, executeExtractMedicationsTask, executeGenerateProviderSummaryTask, executeGeneratePharmacistSummaryTask, executeGenerateDoctorSummaryTask, executeGenerateTravelPackageTask, executeGenerateClinicalSummaryTask, executeGenerateMedicationTimelineTask, executeGenerateConditionTimelineTask, executeGenerateVaccinationHistoryTask, executeTranslatePrescriptionTask, executeFindMedicationEquivalentsTask, executeGenerateMedicationSummaryTask, executeTranslateDocumentsTask, executePreparePassportPackageTask, executePrepareInsuranceSummaryTask, executePrepareVisitSummaryTask, getAllowedTaskTypesForRole, } from './lib/agent/service.js';
import { processDocumentUpload, getDocumentWithExtraction, getUserVaultWithExtraction, updatePassportFromExtractions, } from './lib/vault/intelligence.js';
import { processDocumentPipeline, } from './lib/vault/processor.js';
import { addMemory, getUserMemories, getMemorySummary, deleteMemory, verifyMemory, } from './lib/health-memory/service.js';
import { createCandidates, } from './lib/medical-memory-candidates/service.js';
import { sharePassport, getSharedPassport, getUserSharedPassports, revokeShare, getShareTimeline, generateQrCode, } from './lib/passport/service.js';
import { upsertPresence, getUserPresence, getOnlineUsers, getPresenceForUsers, markMessageRead, getMessageReadReceipts, markConversationMessagesRead, createConversationEvent, getConversationEvents, getUserNotifications, getUnreadNotificationCount, markNotificationRead, markAllNotificationsRead, createActivityItem, getUserActivity, getAllActivity, } from './lib/real-time/service.js';
function parseSessionJsonArr(value) {
...
function severityToRec(severity) {
    switch (severity) {
...
app.use((req, res, next) => {
    ...
    const isAllowed = allowAll || (requestOrigin != null && corsOrigins.includes(requestOrigin));
    if (isAllowed) {
    ...
    try {
        ...
        const session = await prisma.travelHealthSession.update({
            ...
            data: {
                ...
                includedDocuments: Array.isArray(includedDocuments) && includedDocuments.length > 0
                    ? includedDocuments
                    : undefined,
            },
...
app.get('/sessions/:id/interpreter', async (req, res) => {
    try {
        ...
        const english = session.interpreterContext ?? buildInterpreterContext(session, session.user?.healthPassport ?? null, session.user?.healthDocuments ?? []);
        const translated = session.interpreterContextTranslated ?? english;
...
app.get('/tasks/user/:userId', async (req, res) => {
    try {
        ...
        const tasks = await getUserTasks(req.params.userId, { limit, status: status });
        res.json(tasks);
    }
    catch (error) {
        console.error(error);
...
app.get('/memory/:userId', async (req, res) => {
    try {
        ...
        const memories = await getUserMemories(req.params.userId, memoryType);
        res.json(memories);
    }
    catch (error) {
        console.error(error);
...
import { getOrCreateWorkspace, getWorkspace, updateWorkspace, getAllWorkspacesByRole, submitVerification, updateVerificationStatus, getVerifications, getProviderActivityFeed, } from './lib/provider/service.js';
...
import { ensureDataSourcesExist, runIngestion, runAllIngestions, findMedicationEquivalents, searchMedicationGlobally, } from './ingestion/orchestrator.js';
import { getRecentJobs, getDataSourceStats, } from './ingestion/jobs/service.js';
import { findEquivalentsGlobally, searchByBrandName, getCountriesWithData, } from './lib/medications/repository.js';
...
import { createInterpreterSession, getInterpreterSession, addTranscriptEntry, endInterpreterSession, getUserInterpreterSessions, } from './lib/interpreter/session-service.js';
...
app.post('/interpreter-sessions/:id/extract-memory', async (req, res) => {
    try {
        ...
        const transcriptText = transcript
            .map((e) => `${e.role}: ${e.originalText} → ${e.translatedText}`)
            .join('\n');
