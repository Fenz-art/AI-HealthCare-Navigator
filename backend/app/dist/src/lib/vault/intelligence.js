...
async function extractStructuredData(documentId, text) {
...
export async function getDocumentWithExtraction(documentId) {
    return prisma.healthDocument.findUnique({
        where: { id: documentId },
        include: { extraction: true },
    });
}
export async function getUserVaultWithExtraction(userId) {
...
export async function updatePassportFromExtractions(userId) {
        ...
        try {
            if (ext.conditions) {
                ...
                conditions.forEach((c) => {
                    if (c.name && !allConditions.includes(c.name))
                        allConditions.push(c.name);
                });
            }
            if (ext.medications) {
                const meds = JSON.parse(ext.medications);
                meds.forEach((m) => {
                    if (m.name && !allMedications.includes(m.name))
                        allMedications.push(m.name);
                });
            }
            if (ext.allergies) {
                const allergies = JSON.parse(ext.allergies);
                allergies.forEach((a) => {
                    if (a.allergen && !allAllergies.includes(a.allergen))
                        allAllergies.push(a.allergen);
                });
            }
            if (ext.procedures) {
                const procedures = JSON.parse(ext.procedures);
                procedures.forEach((p) => {
                    if (p.name && !allProcedures.includes(p.name))
                        allProcedures.push(p.name);
                });
            }
        }
        catch { }
