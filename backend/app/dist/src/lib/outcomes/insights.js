...
export async function computeRecoveryStatsForJourneys(journeyIds) {
...
export async function generateJourneyInsights(journeyId) {
    ...
    let condition = null;
    try {
        if (Array.isArray(journey.symptoms) && journey.symptoms.length > 0)
            condition = String(journey.symptoms[0]);
    }
    catch {
        // ignore
    }
    ...
    await upsertOutcomeInsightForJourney({
        ...
        city: journey.city,
        medicationId: journey.medicationId,
    ...
    if (journey.medicationId) {
        await generateMedicationInsights(journey.medicationId);
    }
    // (country/user summaries are computed via endpoints using raw data for now)
    return { success: true };
}
export async function generateMedicationInsights(medicationId) {
    const rows = await prisma.healthcareJourney.findMany({
        where: {
...
export async function generateProviderInsights(providerId) {
    const rows = await prisma.healthcareJourney.findMany({
        where: {
