...
function normalizeCandidateInput(m) {
    return {
...
export async function createCandidates(data) {
    if (data.length === 0)
        return { count: 0 };
...
export async function getUserCandidates(userId) {
    return prisma.memoryCandidate.findMany({
        where: { userId },
        orderBy: { createdAt: 'desc' },
    });
}
export async function approveCandidate(candidateId, reviewedByUserId) {
    return prisma.$transaction(async (tx) => {
        ...
        const derivedSourceType = (() => {
            ...
            if (src.startsWith('interpreter-session:'))
                return 'INTERPRETER_SESSION';
            if (src === 'manual')
                return 'MANUAL';
            if (src === 'memory-candidate-approved')
                return 'MEMORY_CANDIDATE_APPROVED';
            return 'UNKNOWN';
        })();
...
export async function rejectCandidate(candidateId, reviewedByUserId) {
    return prisma.$transaction(async (tx) => {
        const candidate = await tx.memoryCandidate.findUnique({
            where: { id: candidateId },
        });
