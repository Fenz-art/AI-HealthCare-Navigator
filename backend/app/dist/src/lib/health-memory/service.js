...
export async function getUserMemories(userId, memoryType) {
...
export async function getMemorySummary(userId) {
    ...
    for (const m of memories) {
        ...
        if (!summary[key])
            summary[key] = [];
        if (!summary[key].includes(m.value))
            summary[key].push(m.value);
    }
    return {
...
async function syncPassportFromMemory(userId) {
