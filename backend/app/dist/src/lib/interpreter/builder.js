...
export function buildInterpreterContext(session, healthPassport, healthDocuments) {
    ...
    const medRecs = parseArr(session.medRecs);
    const providerRecs = parseArr(session.providerRecs);
    const includedDocuments = parseArr(session.includedDocuments);
    ...
    if (session.includedPassport && healthPassport) {
        ...
        const passportAllergies = parseArr(healthPassport.allergies);
        const passportMedications = parseArr(healthPassport.currentMedications);
        const passportConditions = parseArr(healthPassport.chronicConditions);
        const passportVaccinations = parseArr(healthPassport.vaccinations);
        const emergencyContacts = parseArr(healthPassport.emergencyContacts);
