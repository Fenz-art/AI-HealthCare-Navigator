...
const PIPELINE_STEPS = ['STORING', 'OCR', 'DETECTING_LANGUAGE', 'TRANSLATING', 'EXTRACTING', 'UPDATING_MEMORY', 'UPDATING_PASSPORT'] as const;
...
async function updateJobStep(jobId: string, step: string, progress: number) {
...
export async function processDocumentPipeline(data: {
  ...
  const result = { document, job, translation: null as any, extraction: null as any, memories: null as any, passport: null as any };
...
async function detectLanguage(text: string, hint: string): Promise<string> {
  if (hint && hint !== 'unknown' && hint !== 'auto') return hint;
  try {
...
async function extractStructuredData(documentId: string, text: string, language: string) {
...
async function seedMedicalMemory(
  userId: string,
  documentId: string,
  extraction: any,
  ...
  const memories: { memoryType: MemoryType; value: string; source: string; verified: boolean }[] = [];
