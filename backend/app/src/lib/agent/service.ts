...
import { addMemories, getUserMemories, getMemorySummary } from '../health-memory/service.js';
...
export async function getTaskWithDocument(taskId: string) {
...
export async function getUserTasks(userId: string, options?: { limit?: number; status?: TaskStatus }) {
...
export async function getAllUserTasksByStatus(userId: string) {
...
export async function updateTaskProgress(taskId: string, data: { progress?: number; status?: TaskStatus; output?: Record<string, unknown>; error?: string }) {
...
export async function executeTranslateDocumentTask(taskId: string) {
  const task = await prisma.agentTask.findUnique({
    where: { id: taskId },
    include: { document: true },
  });
  if (!task || !task.document) throw new Error('Task or document not found');
...
export async function executeExtractMedicalDataTask(taskId: string) {
  const task = await prisma.agentTask.findUnique({
    where: { id: taskId },
    include: { document: true },
  });
...
export async function executeGenerateSummaryTask(taskId: string) {
  const task = await prisma.agentTask.findUnique({
    where: { id: taskId },
    include: { document: true },
  });
...
export async function executeExtractConditionsTask(taskId: string) {
  const task = await prisma.agentTask.findUnique({
    where: { id: taskId },
    include: { document: true },
  });
...
${typeof text === 'string' ? text : JSON.stringify(text)}`;
  ...
  try {
    ...
    if (Array.isArray(parsed) && parsed.length > 0) {
      const memories = parsed.filter((c: any) => c.name).map((c: any) => ({
        memoryType: 'CONDITION' as const,
        value: c.name,
        source: `task:${taskId}`,
        verified: false,
      }));
...
export async function executeExtractMedicationsTask(taskId: string) {
  const task = await prisma.agentTask.findUnique({
    where: { id: taskId },
    include: { document: true },
  });
...
${typeof text === 'string' ? text : JSON.stringify(text)}`;
  ...
  try {
    ...
    if (Array.isArray(parsed) && parsed.length > 0) {
      const memories = parsed.filter((m: any) => m.name).map((m: any) => ({
        memoryType: 'MEDICATION' as const,
...
export async function executeGenerateProviderSummaryTask(taskId: string) {
  await updateTaskProgress(taskId, { status: 'PROCESSING', progress: 10 });
...
export async function executeGeneratePharmacistSummaryTask(taskId: string) {
  await updateTaskProgress(taskId, { status: 'PROCESSING', progress: 10 });
...
export async function executeGenerateDoctorSummaryTask(taskId: string) {
  await updateTaskProgress(taskId, { status: 'PROCESSING', progress: 10 });
...
export async function executeGenerateTravelPackageTask(taskId: string) {
  await updateTaskProgress(taskId, { status: 'PROCESSING', progress: 10 });
  ...
  const destination = (input as any).destination || 'your destination';
...
export async function executeGenerateClinicalSummaryTask(taskId: string) {
  await updateTaskProgress(taskId, { status: 'PROCESSING', progress: 10 });
...
export async function executeGenerateMedicationTimelineTask(taskId: string) {
  await updateTaskProgress(taskId, { status: 'PROCESSING', progress: 10 });
...
export async function executeGenerateConditionTimelineTask(taskId: string) {
  await updateTaskProgress(taskId, { status: 'PROCESSING', progress: 10 });
...
export async function executeGenerateVaccinationHistoryTask(taskId: string) {
  await updateTaskProgress(taskId, { status: 'PROCESSING', progress: 10 });
...
export async function executeTranslatePrescriptionTask(taskId: string) {
  const task = await prisma.agentTask.findUnique({
    where: { id: taskId },
    include: { document: true },
  });
...
export async function executeFindMedicationEquivalentsTask(taskId: string) {
  await updateTaskProgress(taskId, { status: 'PROCESSING', progress: 10 });
...
export async function executeGenerateMedicationSummaryTask(taskId: string) {
  await updateTaskProgress(taskId, { status: 'PROCESSING', progress: 10 });
...
export async function executeTranslateDocumentsTask(taskId: string) {
  const task = await prisma.agentTask.findUnique({
    where: { id: taskId },
    include: { document: true },
  });
...
export async function executePreparePassportPackageTask(taskId: string) {
  await updateTaskProgress(taskId, { status: 'PROCESSING', progress: 10 });
...
export async function executePrepareInsuranceSummaryTask(taskId: string) {
  await updateTaskProgress(taskId, { status: 'PROCESSING', progress: 10 });
...
export async function executePrepareVisitSummaryTask(taskId: string) {
  await updateTaskProgress(taskId, { status: 'PROCESSING', progress: 10 });
...
export function getAllowedTaskTypesForRole(role: string): TaskType[] {
  switch (role) {
    case 'DOCTOR':
