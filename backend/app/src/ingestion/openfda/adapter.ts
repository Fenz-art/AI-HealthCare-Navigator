...
export async function ingestOpenFDA(dataSourceId: string): Promise<IngestionResult> {
  ...
  try {
    ...
    while (skip < MAX_RECORDS) {
      ...
      const data = await response.json() as { results?: OpenFDAResult[]; error?: { message: string } };
