...
export async function createJob(dataSourceId: string, sourceType: IngestionSourceType): Promise<string> {
...
export async function completeJob(jobId: string, recordsImported: number, recordsFailed: number): Promise<void> {
  await prisma.ingestionJob.update({
    where: { id: jobId },
    data: {
      status: 'COMPLETED',
