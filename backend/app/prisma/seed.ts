...
// ── 3. Brand mappings ─────────────────────────────────────────────
type M = { i: string; c: string; b: string; s: string; d?: string };
const MAPPINGS: M[] = [
  // Paracetamol
...
async function main() {
  ...
  const ingValues = INGREDIENTS.map(name =>
    `('${cuid()}','${name.replace(/'/g,"''")}')`)
    .join(',');
  ...
  for (const m of MAPPINGS) {
    ...
    const brandId = cuid();
    brandRows.push(
      `('${brandId}','${m.b.replace(/'/g,"''")}','${country.id}','${ingredient.id}','${m.s}','${m.d ?? 'STANDARD_OTC'}')`
    );
    edgeRows.push(
      `('${cuid()}','${ingredient.id}','${country.id}','${brandId}')`
    );
  }
  ...
  const safeEdgeRows: string[] = [];
  for (const m of MAPPINGS) {
    const country    = countryByCode.get(m.c);
    const ingredient = ingByName.get(m.i);
    if (!country || !ingredient) continue;
